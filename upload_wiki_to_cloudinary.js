...
mongoose.connect(process.env.DATABASE).then(async () => {
  ...
  for (let i = 0; i < tours.length; i++) {
    ...
    console.log(`Updated tour ${t.name} with ${newImages.length} real images from ${rKey}`);
  }
  console.log("Successfully migrated ALL Wikipedia landmark images to Cloudinary!");
  process.exit(0);
});
