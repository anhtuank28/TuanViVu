...
function downloadImage(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0 TourManager/1.0' } }, (response) => {
...
async function uploadToCloudinary(url) {
  ...
  try {
    await sleep(500); // Prevent Wikipedia 429
...
mongoose.connect(process.env.DATABASE).then(async () => {
  ...
  for (let i = 0; i < tours.length; i++) {
    ...
    if (newImages.length === 0) {
       console.log(`Failed totally for ${t.name}, skipping update to prevent empty array!`);
       continue;
    }
  ...
  console.log("Successfully migrated ALL Wikipedia landmark images to Cloudinary, SLOWLY!");
  process.exit(0);
});
